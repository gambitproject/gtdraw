...
def commandline(argv: List[str]) -> tuple[str, bool, bool, bool, bool, Optional[str], Optional[int]]:
    ...
    command-line arguments. Also detects if PDF, PNG, or SVG output is requested.
...
def generate_svg(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if isinstance(e, FileNotFoundError) or (isinstance(e, subprocess.CalledProcessError) and "pdf2svg" in str(e.cmd)):
                raise RuntimeError(
                    "SVG conversion failed. Please install pdf2svg.\n"
                    "Installation examples:\n"
                    "  macOS: brew install pdf2svg\n"
                    "  Ubuntu: sudo apt-get install pdf2svg\n"
                    "  Windows: Install pdf2svg from its website or via package manager"
                ) from e
            raise
        except RuntimeError:
            # Re-raise PDF generation errors
