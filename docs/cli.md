...
By default, `efgviz` generates TikZ code and prints it to standard output.
...
efgviz games/example.ef --to-efg

# Convert EFG to EF
efgviz games/efg/example.efg --to-ef

# Specify output filename
efgviz games/example.ef --to-efg --output=my_game.efg
```
...
efgviz games/nfg/example.nfg

# Compile payoff table to PDF
efgviz games/nfg/example.nfg --pdf

# Compile payoff table to PNG
efgviz games/nfg/example.nfg --png

# Compile payoff table to SVG with custom output name
efgviz games/nfg/example.nfg --svg --output=battle_of_sexes.svg
```