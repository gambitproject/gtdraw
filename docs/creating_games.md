...
EFGViz cannot be used to generate games directly. Games can be specified via `.ef` [format](ef_format.md) files which include layout formatting, or via Gambit's `.efg` [format](https://gambitproject.readthedocs.io/en/stable/formats.efg.html) files (requires `pygambit`).
...
EFGViz also supports **Normal Form Games** (`.nfg` files) — see the [NFG section](#normal-form-games-nfg) below.

EFGViz serves as a bridge between game theory file formats and high-quality visualizations. The diagram below illustrates how data flows from various sources through EFGViz to produce publication-ready graphics.
...
flowchart TD
    ...
    style EFGViz fill:#fff,stroke:#333,stroke-dasharray: 5 5
...
2. **Formats**: 
   - **EF**: The native EFGViz format, optimized for manual layout and TikZ rendering.
...
4. **Generation**: EFGViz can natively ingest **either EF or EFG** files to generate output. 
...
EFGViz renders normal form (strategic form) games from Gambit's `.nfg` file format or from `pygambit` NFG game objects.
